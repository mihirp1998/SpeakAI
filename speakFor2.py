...
def dense():
	...
	model.add(Dense(100,input_dim=399168,activation='elu' ,kernel_regularizer=regularizers.l2(0.00001),
...
def model_train(x_train,y_train,x_test, y_test, model):
	...
	histories = modelcheck2.Histories("/data/Mihir/SpeakAI_data2/models/ahaan3.hdf5",verbose = 1,  mode='max', monitor='val_acc', save_best_only=True,save_weights_only=True)
...
def savedata(model): 
	...
	with open(filePath + "/SpeakAI_data2/speaker2.json", "w") as json_file:
    		json_file.write(model_json)
    	model.save_weights(filePath + "/SpeakAI_data2/speaker2.h5")
...
def preload():
	...
	mod = model()
	rootdir = '/SpeakAI_data2/data/train/'
	traindata = training_data(rootdir)
	x_train = traindata[0]
	y_train = traindata[1]
	rootdir1 = '/SpeakAI_data2/data/test/'
	...
	svm_y_test = np_utils.to_categorical(svm_y_test,2)
	svm_y_train = np_utils.to_categorical(svm_y_train,2)
	# train scaler
	# scaler = StandardScaler()
	# scaler.fit(svm_x_train)
	# pickle.dump(scaler,open('scaler2.p','wb'))
	...
	pickle.dump(svm_x_train,open(filePath + '/SpeakAI_data2/x_train2.p','wb'))
	pickle.dump(svm_y_train,open(filePath + '/SpeakAI_data2/y_train2.p','wb'))
	pickle.dump(svm_x_test,open(filePath + '/SpeakAI_data2/x_test2.p','wb'))
	pickle.dump(svm_y_test,open(filePath + '/SpeakAI_data2/y_test2.p','wb'))
def dataPreload():
	global svm_x_train,svm_y_train,svm_x_test,svm_y_test
	svm_x_train = pickle.load(open(filePath + '/SpeakAI_data2/x_train2.p','rb'))
	svm_y_train = pickle.load(open(filePath + '/SpeakAI_data2/y_train2.p','rb'))
	svm_x_test = pickle.load(open(filePath + '/SpeakAI_data2/x_test2.p','rb'))
	svm_y_test = pickle.load(open(filePath + '/SpeakAI_data2/y_test2.p','rb'))
