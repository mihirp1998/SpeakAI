...
def dense():
	...
	model.add(Dense(100,input_dim=399168,activation='elu' ,kernel_regularizer=regularizers.l2(0.0001),
                activity_regularizer=regularizers.l1(0.000000001)))
	model.add(Dropout(0.25))
...
def model_train(x_train,y_train,x_test, y_test, model):
	...
	histories = modelcheck.Histories("/data/Mihir/SpeakAI_data/models/a_newbestmodel.hdf5",verbose = 1,  mode='max', monitor='val_acc', save_best_only=True,save_weights_only=True)
...
def model_test(x_test, y_test, x_train, y_train):
	...
	loaded_model.load_weights(filePath + "/SpeakAI_data/models/kishan.hdf5")
...
def preload():
	...
	x_train = traindata[0]
	y_train = traindata[1]
	rootdir1 = filePath + '/SpeakAI_data/data/testdatafor2/'
	...
	print(svm_x_test.shape,svm_y_test.shape)
	...
	pickle.dump(svm_x_test,open(filePath + '/SpeakAI_data/x_test_for_2.p','wb'))
	pickle.dump(svm_y_test,open(filePath + '/SpeakAI_data/y_test_for_2.p','wb'))
...
def dataPreload():
	...
	# svm_x_test = pickle.load(open(filePath + '/SpeakAI_data/x_test_for_2.p','rb'))
	# svm_y_test = pickle.load(open(filePath + '/SpeakAI_data/y_test_for_2.p','rb'))
